...
#Descrizione dell programmaesercizio5.py:
...
def stessa_diagonale(x0, y0, x1, y1):                           
    ...
    if dx==dy:                                                  #se la differenza tra le coordinate x è uguale alla differenza tra le coordinate y, allora allora le posizioni sono sulla stessa diagonale, quindi ritorno True
        return True                                             
    else:                                                       #altrimenti non sono sulla stessa diagonale, quindi ritorno False   
        return False                                            
...
def soluzioni_da_dati_utente(dimensionescacchiera, numerosoluzioni):
    ...
    while soluzioni_trovate < numerosoluzioni:                              #continuo a cercare soluzioni finché il numero di soluzioni uniche trovate è inferiore al numero di soluzioni desiderate inserito dall'utente
        ...
        while incrocia_colonne(lista_soluzione):                            #verifico se la permutazione generata è una soluzione valida per l'algoritmo delle n regine,se la permutazione non è una soluzione valida, genero una nuova permutazione casuale e incremento il contatore dei tentativi totali
        ...
        if tuple(lista_soluzione) not in soluzioni_uniche:                                  #se la permutazione generata è una soluzione valida e non è già presente nel set delle soluzioni uniche
            ...
            if soluzioni_ripetute > numerosoluzioni * 10:                                   #se ci sono troppe soluzioni ripetute, interrompiamo per evitare un loop infinito
                print("Troppe soluzioni ripetute, interrompo la ricerca.")                  #avviso l'utente che ci sono troppe soluzioni ripetute e interrompo la ricerca, per evitare un loop infinito in cui vengono generate solo soluzioni duplicate senza trovare nuove soluzioni uniche e interrompo il ciclo while
                break
    ...
    print(f"Tempo medio per soluzione: {(end - start) / numerosoluzioni:.4f} secondi")      #stampo il tempo medio per soluzione, calcolato come il tempo totale diviso per il numero di soluzioni uniche trovate, così da avere un'indicazione del tempo medio per trovare una soluzione unica
...
def dimensione_massima_per_tempo_limite():
    ...
    while incrocia_colonne(lista_soluzione):                                                #verifico se la soluzione iniziale è valida, se non lo è genero permutazioni casuali finché non trovo una soluzione valida
        random_generator.shuffle(lista_soluzione)                                           
    ...
    while (end - start) < tempo_limite:                                                     #se il tempo impiegato è < di 15s
        ...
        while incrocia_colonne(lista_soluzione):                                            #verifico se la soluzione iniziale è valida per la nuova dimensione della scacchiera, se non lo è genero permutazioni casuali finché non trovo una soluzione valida per la nuova dimensione della scacchiera
            random_generator.shuffle(lista_soluzione)                                       
...
def rotazione_90_gradi(soluzione):
    ...
    for r, c in enumerate(soluzione):                                          #itero su ogni elemento della soluzione, dove r rappresenta la riga e c rappresenta la colonna in cui si trova la regina, la funzione enumerate mi permette di ottenere sia l'indice r che il valore c per ogni elemento della soluzione
        soluzione90[c] = (len(soluzione)-1)-r                                  #con una rotazione di 90 gradi in senso orario, colonne e righe si scambiano, quindi la regina che era alla riga r e colonna c, dopo la rotazione sarà alla riga c e colonna n-1-r, dove n è la dimensione della scacchiera
...
def rotazione_180_gradi(soluzione):
    ...
    for col in range(n):                                                    #itero su ogni colonna della soluzione, dove col rappresenta la colonna in cui si trova la regina
        soluzione180[(n-1) - col] = (n-1) - soluzione[col]                  #soluzione180[(n-1) - col]: Assegna la regina alla nuova colonna (se era in colonna col, dopo una rotazione di 180 gradi sarà in colonna n-1-col),
                                                                            #soluzione[col]: Recupera il valore della riga dove si trova la regina nella colonna col
                                                                            #(n-1) - soluzione[col]: Calcola la nuova riga dopo la rotazione, (se la regina era alla riga r, dopo una rotazione di 180 gradi sarà alla riga n-1-r)
...
def rotazione_270_gradi(soluzione):
    ...
    for r,c in enumerate(soluzione):                                        #itero su ogni elemento della soluzione, dove r rappresenta la riga e c rappresenta la colonna in cui si trova la regina
        soluzione270[(len(soluzione) - 1) - c] = r                          #con una rotazione di 270 gradi in senso orario, colonne e righe si scambiano, come se la scacchiera venisse ruotata di 90 gradi in senso antiorario, quindi la regina che era alla riga r e colonna c, dopo la rotazione sarà alla riga n-1-c e colonna r, dove n è la dimensione della scacchiera
...
def main():
    ...
    print("\nSoluzioni uniche e rispettive soluzioni simmetriche per rotazione per una scacchiera 8x8:\n") #stampo a video un messaggio che indica che successivamente verranno stampate le soluzioni uniche e le rispettive soluzioni simmetriche
