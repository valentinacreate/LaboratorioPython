...
#Descrizione del programma esercizio2.py:
...
def specchio(frasi):
    ...
    for i in range(len(frasi)):                         #scorro tutte le frasi
        if i == 2 or i == 7 or i == 11:                 #il secondo verso di ogni strofa è l'indice 1, 5, 9,..., creo una lista di tutti i caratteri della frase corrente e una lista vuota per contenere la frase con i caratteri invertiti
            caratteri = list(frasi[i])                  
            frase_specchiata = []                            
            for j in range(len(caratteri)-1,-1,-1):     #scorro tutti i caratteri della frase, partendo dall'ultimo carattere (indice len(caratteri)-1) fino al primo carattere (indice 0), appendo il carattere corrente alla lista della frase specchiata, in questo modo i caratteri vengono appesi in ordine inverso rispetto all'ordine originale
